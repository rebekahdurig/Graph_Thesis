...
In essence, Dr. Kocik took basic graph theory (node/edge structures) and added a binary state to each node, as well as a discrete time function. This creates a "Game of Light" (similar to Conway's "Game of Life") Markov chain structure. Essentially, if a node is not excited, but has an odd number of excited neighbors, it will excite in the next step. If it is excited and has an odd number of excited neigbors, it will stay excited, otherwise, it will go to rest. The papers below discuss more of the mathematical implications of this game.
...
### Process
First, some of this program was vibe-coded in Claude. I explained the broad outline of what I wanted it to do, and was talking to it about options on how to code a GUI in Python, perhaps using Processing. Most of my work in Python has been based around data/Pandas so far. I used Processing to code my original GUI, and it seemed like there may have been an update where it would support Python, but after some experimentation, that didn't seem like a good option. Claude ended up writing the basic structure of the GUI, but left a lot of the implementation of the Game of Light to me.
